...
function normalizeApiBase(raw) {
    ...
    if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
        throw new Error(`COCO_API_BASE must start with http:// or https://, got "${candidate}"`);
    }
    if (!parsed.hostname) {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        if (name === "coco.task.dependencies.create") {
            ...
            if (typeof args?.depends_on_task_id !== "number" &&
        ...
        if (name === "coco.task.dependencies.delete") {
            ...
            const result = await fetchJson(`/api/v2/tasks/${taskId}/dependencies/${dependsOnTaskId}`, {
                method: "DELETE",
            });
            return {
                content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
