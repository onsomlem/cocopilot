...
function activate(context) {
    ...
    const getApiHeaders = (extra) => {
        ...
        const apiKey = cocopilotConfig.get("apiKey") || "";
        const headers = { ...extra };
        if (apiKey.trim()) {
    ...
    const mcpStatusBar = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 99);
    const updateStatusBar = () => {
        const cocopilotConfig = vscode.workspace.getConfiguration("cocopilot");
        const projectId = cocopilotConfig.get("projectId") || "";
        const label = projectId.trim() ? projectId.trim() : "unset";
        statusBar.text = `Cocopilot: ${label}`;
        statusBar.show();
    };
    const updateMcpStatusBar = (isRunning) => {
        if (isRunning) {
    ...
    const disposable = vscode.commands.registerCommand("cocopilot.hello", () => {
        vscode.window.showInformationMessage("Hello from Cocopilot");
    });
    const configureMcp = vscode.commands.registerCommand("cocopilot.configureMcp", async () => {
        ...
        try {
            const cocopilotConfig = vscode.workspace.getConfiguration("cocopilot");
            const apiBase = cocopilotConfig.get("apiBase") || "<COCO_API_BASE>";
            const projectId = cocopilotConfig.get("projectId") || "<COCO_PROJECT_ID>";
            const apiKey = cocopilotConfig.get("apiKey") || "";
    ...
    const ensureMcpConfig = async (workspaceFolder) => {
        const vscodeDir = vscode.Uri.joinPath(workspaceFolder.uri, ".vscode");
        await vscode.workspace.fs.createDirectory(vscodeDir);
        const configUri = vscode.Uri.joinPath(vscodeDir, "mcp.json");
        try {
    ...
    const openMcpConfig = vscode.commands.registerCommand("cocopilot.openMcpConfig", async () => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            vscode.window.showErrorMessage("Open a workspace folder first.");
            return;
    ...
    const startMcpServerNow = async (showAlreadyRunningMessage) => {
        ...
        const cocopilotConfig = vscode.workspace.getConfiguration("cocopilot");
        const apiBase = cocopilotConfig.get("apiBase") || "http://localhost:8080";
        const projectId = cocopilotConfig.get("projectId") || "";
        if (mcpTerminal) {
            mcpTerminal.show();
    ...
    const startMcpServer = vscode.commands.registerCommand("cocopilot.startMcpServer", async () => {
        await startMcpServerNow(true);
    });
    const stopMcpServer = vscode.commands.registerCommand("cocopilot.stopMcpServer", async () => {
    ...
    const openDashboard = vscode.commands.registerCommand("cocopilot.openDashboard", async () => {
        const targetUrl = buildUiUrl();
        const target = vscode.Uri.parse(targetUrl);
        await vscode.env.openExternal(target);
    });
    const openTasksBoard = vscode.commands.registerCommand("cocopilot.openTasksBoard", async () => {
        const targetUrl = buildUiUrl("/");
        const target = vscode.Uri.parse(targetUrl);
        await vscode.env.openExternal(target);
    });
    const openHealth = vscode.commands.registerCommand("cocopilot.openHealth", async () => {
        const cocopilotConfig = vscode.workspace.getConfiguration("cocopilot");
        const apiBase = cocopilotConfig.get("apiBase") || "http://localhost:8080";
        let targetUrl = "";
        try {
    ...
    const openConfig = vscode.commands.registerCommand("cocopilot.openConfig", async () => {
        const cocopilotConfig = vscode.workspace.getConfiguration("cocopilot");
        const apiBase = cocopilotConfig.get("apiBase") || "http://localhost:8080";
        let targetUrl = "";
        try {
    ...
    const openVersion = vscode.commands.registerCommand("cocopilot.openVersion", async () => {
        const cocopilotConfig = vscode.workspace.getConfiguration("cocopilot");
        const apiBase = cocopilotConfig.get("apiBase") || "http://localhost:8080";
        let targetUrl = "";
        try {
    ...
    const openEvents = vscode.commands.registerCommand("cocopilot.openEvents", async () => {
        const targetUrl = buildUiUrl("/api/v2/events");
        const target = vscode.Uri.parse(targetUrl);
        await vscode.env.openExternal(target);
    });
    const openEventsStream = vscode.commands.registerCommand("cocopilot.openEventsStream", async () => {
        const cocopilotConfig = vscode.workspace.getConfiguration("cocopilot");
        const apiBase = cocopilotConfig.get("apiBase") || "http://localhost:8080";
        const defaultProjectId = cocopilotConfig.get("projectId") || "";
    ...
    const openAgents = vscode.commands.registerCommand("cocopilot.openAgents", async () => {
        const targetUrl = buildUiUrl("/api/v2/agents");
        const target = vscode.Uri.parse(targetUrl);
        await vscode.env.openExternal(target);
    });
    const openLeases = vscode.commands.registerCommand("cocopilot.openLeases", async () => {
        const cocopilotConfig = vscode.workspace.getConfiguration("cocopilot");
        const apiBase = cocopilotConfig.get("apiBase") || "http://localhost:8080";
        let targetUrl = "";
        try {
    ...
    const openTasksApi = vscode.commands.registerCommand("cocopilot.openTasksApi", async () => {
        const targetUrl = buildUiUrl("/api/v2/tasks");
        const target = vscode.Uri.parse(targetUrl);
        await vscode.env.openExternal(target);
    });
    const openTaskDetail = vscode.commands.registerCommand("cocopilot.openTaskDetail", async () => {
    ...
    const openRunDetail = vscode.commands.registerCommand("cocopilot.openRunDetail", async () => {
    ...
    const openProjectDetail = vscode.commands.registerCommand("cocopilot.openProjectDetail", async () => {
        const projectIdInput = await vscode.window.showInputBox({
    ...
    const openProjectTree = vscode.commands.registerCommand("cocopilot.openProjectTree", async () => {
        const projectIdInput = await vscode.window.showInputBox({
            prompt: "Enter project id",
            placeHolder: "proj_123",
            ignoreFocusOut: true
    ...
    const openProjectAudit = vscode.commands.registerCommand("cocopilot.openProjectAudit", async () => {
        const projectIdInput = await vscode.window.showInputBox({
            prompt: "Enter project id",
            placeHolder: "proj_123",
            ignoreFocusOut: true
    ...
    const openProjectChanges = vscode.commands.registerCommand("cocopilot.openProjectChanges", async () => {
        const projectIdInput = await vscode.window.showInputBox({
            prompt: "Enter project id",
            placeHolder: "proj_123",
            ignoreFocusOut: true
        ...
        try {
            const baseUrl = new URL(apiBase);
            const target = new URL(`/api/v2/projects/${encodedProjectId}/changes`, baseUrl);
            if (since) {
    ...
    const openProjectMemory = vscode.commands.registerCommand("cocopilot.openProjectMemory", async () => {
        const projectIdInput = await vscode.window.showInputBox({
            prompt: "Enter project id",
            placeHolder: "proj_123",
            ignoreFocusOut: true
        ...
        try {
            const baseUrl = new URL(apiBase);
            const target = new URL(`/api/v2/projects/${encodedProjectId}/memory`, baseUrl);
            if (scope) {
    ...
    const openProjectEventsReplay = vscode.commands.registerCommand("cocopilot.openProjectEventsReplay", async () => {
        const projectIdInput = await vscode.window.showInputBox({
            prompt: "Enter project id",
            placeHolder: "proj_123",
            ignoreFocusOut: true
        ...
        try {
            const baseUrl = new URL(apiBase);
            const target = new URL(`/api/v2/projects/${encodedProjectId}/events/replay`, baseUrl);
            if (sinceId) {
    ...
    const openPolicies = vscode.commands.registerCommand("cocopilot.openPolicies", async () => {
        const cocopilotConfig = vscode.workspace.getConfiguration("cocopilot");
        const configuredProjectId = cocopilotConfig.get("projectId") || "";
        ...
        try {
            const baseUrl = new URL(apiBase);
            targetUrl = new URL(`/api/v2/projects/${encodedProjectId}/policies`, baseUrl).toString();
        }
        catch {
            const trimmed = apiBase.replace(/\/+$/, "");
    ...
    const openPolicyDetail = vscode.commands.registerCommand("cocopilot.openPolicyDetail", async () => {
        const cocopilotConfig = vscode.workspace.getConfiguration("cocopilot");
        const configuredProjectId = cocopilotConfig.get("projectId") || "";
        let projectId = configuredProjectId.trim();
        if (!projectId) {
        ...
        try {
            const baseUrl = new URL(apiBase);
            targetUrl = new URL(`/api/v2/projects/${encodedProjectId}/policies/${encodedPolicyId}`, baseUrl).toString();
        }
        catch {
            const trimmed = apiBase.replace(/\/+$/, "");
    ...
    const listPolicies = vscode.commands.registerCommand("cocopilot.listPolicies", async () => {
        const cocopilotConfig = vscode.workspace.getConfiguration("cocopilot");
        const configuredProjectId = cocopilotConfig.get("projectId") || "";
        let projectId = configuredProjectId.trim();
        if (!projectId) {
        ...
        try {
            ...
            if (!response.ok) {
                vscode.window.showErrorMessage(`Failed to list policies (${response.status}).`);
                outputChannel.appendLine(`[VSIX] list policies failed (${response.status}): ${responseText}`);
                return;
            }
    ...
    const createPolicy = vscode.commands.registerCommand("cocopilot.createPolicy", async () => {
        const cocopilotConfig = vscode.workspace.getConfiguration("cocopilot");
        const configuredProjectId = cocopilotConfig.get("projectId") || "";
        let projectId = configuredProjectId.trim();
        if (!projectId) {
        ...
        outputChannel.appendLine(`[VSIX] create policy POST ${targetUrl} project_id=${projectId}`);
        try {
            ...
            const responseText = await response.text();
            if (!response.ok) {
                vscode.window.showErrorMessage(`Failed to create policy (${response.status}).`);
                outputChannel.appendLine(`[VSIX] create policy failed (${response.status}): ${responseText}`);
                return;
            }
            let payload;
            try {
    ...
    const updatePolicy = vscode.commands.registerCommand("cocopilot.updatePolicy", async () => {
        const cocopilotConfig = vscode.workspace.getConfiguration("cocopilot");
        const configuredProjectId = cocopilotConfig.get("projectId") || "";
        let projectId = configuredProjectId.trim();
        if (!projectId) {
        ...
        outputChannel.appendLine(`[VSIX] update policy PATCH ${targetUrl} project_id=${projectId} policy_id=${policyId}`);
        try {
            const response = await fetch(targetUrl, {
                method: "PATCH",
                headers: getApiHeaders({
            ...
            const responseText = await response.text();
            if (!response.ok) {
                vscode.window.showErrorMessage(`Failed to update policy (${response.status}).`);
                outputChannel.appendLine(`[VSIX] update policy failed (${response.status}): ${responseText}`);
                return;
            }
    ...
    const togglePolicy = vscode.commands.registerCommand("cocopilot.togglePolicy", async () => {
        const cocopilotConfig = vscode.workspace.getConfiguration("cocopilot");
        const configuredProjectId = cocopilotConfig.get("projectId") || "";
        let projectId = configuredProjectId.trim();
        if (!projectId) {
        ...
        outputChannel.appendLine(`[VSIX] toggle policy PATCH ${targetUrl} project_id=${projectId} policy_id=${policyId}`);
        try {
            const response = await fetch(targetUrl, {
                method: "PATCH",
                headers: getApiHeaders({
            ...
            const responseText = await response.text();
            if (!response.ok) {
                vscode.window.showErrorMessage(`Failed to toggle policy (${response.status}).`);
                outputChannel.appendLine(`[VSIX] toggle policy failed (${response.status}): ${responseText}`);
                return;
            }
            const trimmed = responseText.trim();
            const messageText = trimmed
    ...
    const deletePolicy = vscode.commands.registerCommand("cocopilot.deletePolicy", async () => {
        const cocopilotConfig = vscode.workspace.getConfiguration("cocopilot");
        const configuredProjectId = cocopilotConfig.get("projectId") || "";
        let projectId = configuredProjectId.trim();
        if (!projectId) {
        ...
        outputChannel.appendLine(`[VSIX] delete policy DELETE ${targetUrl} project_id=${projectId} policy_id=${policyId}`);
        try {
            const response = await fetch(targetUrl, { method: "DELETE", headers: getApiHeaders() });
            const responseText = await response.text();
            if (!response.ok) {
                vscode.window.showErrorMessage(`Failed to delete policy (${response.status}).`);
                outputChannel.appendLine(`[VSIX] delete policy failed (${response.status}): ${responseText}`);
                return;
            }
            const trimmed = responseText.trim();
            const messageText = trimmed
    ...
    const createTask = vscode.commands.registerCommand("cocopilot.createTask", async () => {
        ...
        try {
            ...
            const responseText = await response.text();
            if (!response.ok) {
                vscode.window.showErrorMessage(`Failed to create task (${response.status}).`);
                outputChannel.appendLine(`[VSIX] create task failed (${response.status}): ${responseText}`);
                return;
            }
            let payload;
            try {
            ...
            if (taskId === undefined || taskId === null) {
                vscode.window.showInformationMessage("Task created, but no task id was returned.");
                outputChannel.appendLine(`[VSIX] create task response: ${responseText}`);
                return;
            }
    ...
    const saveTask = vscode.commands.registerCommand("cocopilot.saveTask", async () => {
        const taskIdInput = await vscode.window.showInputBox({
            prompt: "Enter task id",
            placeHolder: "123",
            ignoreFocusOut: true
        ...
        try {
            ...
            const responseText = await response.text();
            if (!response.ok) {
                vscode.window.showErrorMessage(`Failed to complete task (${response.status}).`);
                outputChannel.appendLine(`[VSIX] complete task failed (${response.status}): ${responseText}`);
                return;
            }
    ...
    const failTask = vscode.commands.registerCommand("cocopilot.failTask", async () => {
        const taskIdInput = await vscode.window.showInputBox({
        ...
        try {
            ...
            const responseText = await response.text();
            if (!response.ok) {
                vscode.window.showErrorMessage(`Failed to fail task (${response.status}).`);
                outputChannel.appendLine(`[VSIX] fail task failed (${response.status}): ${responseText}`);
                return;
            }
    ...
    const updateStatus = vscode.commands.registerCommand("cocopilot.updateStatus", async () => {
        const taskIdInput = await vscode.window.showInputBox({
            prompt: "Enter task id",
            placeHolder: "123",
            ignoreFocusOut: true
        ...
        outputChannel.appendLine(`[VSIX] update status PATCH ${targetUrl} task_id=${taskId} status=${status}`);
        try {
            const response = await fetch(targetUrl, {
                method: "PATCH",
                headers: getApiHeaders({
            ...
            const responseText = await response.text();
            if (!response.ok) {
                vscode.window.showErrorMessage(`Failed to update status (${response.status}).`);
                outputChannel.appendLine(`[VSIX] update status failed (${response.status}): ${responseText}`);
                return;
            }
            const trimmed = responseText.trim();
            const messageText = trimmed
    ...
    const updateTask = vscode.commands.registerCommand("cocopilot.updateTask", async () => {
        const taskIdInput = await vscode.window.showInputBox({
            prompt: "Enter task id",
            placeHolder: "123",
            ignoreFocusOut: true
        ...
        try {
            ...
            payload = Object.entries(parsed).reduce((acc, [key, value]) => {
        ...
        try {
            ...
            const responseText = await response.text();
            if (!response.ok) {
                vscode.window.showErrorMessage(`Failed to update task (${response.status}).`);
                outputChannel.appendLine(`[VSIX] update task failed (${response.status}): ${responseText}`);
                return;
            }
            const trimmed = responseText.trim();
            const messageText = trimmed
    ...
    const deleteTask = vscode.commands.registerCommand("cocopilot.deleteTask", async () => {
        const taskIdInput = await vscode.window.showInputBox({
            prompt: "Enter task id",
            placeHolder: "123",
            ignoreFocusOut: true
        ...
        try {
            ...
            const responseText = await response.text();
            if (!response.ok) {
                vscode.window.showErrorMessage(`Failed to delete task (${response.status}).`);
                outputChannel.appendLine(`[VSIX] delete task failed (${response.status}): ${responseText}`);
                return;
            }
            const trimmed = responseText.trim();
            const messageText = trimmed
    ...
    const listTasks = vscode.commands.registerCommand("cocopilot.listTasks", async () => {
        const cocopilotConfig = vscode.workspace.getConfiguration("cocopilot");
        const apiBase = cocopilotConfig.get("apiBase") || "http://localhost:8080";
        const defaultProjectId = cocopilotConfig.get("projectId") || "";
        const statusInput = await vscode.window.showInputBox({
        ...
        try {
            ...
            const responseText = await response.text();
            if (!response.ok) {
                vscode.window.showErrorMessage(`Failed to list tasks (${response.status}).`);
                outputChannel.appendLine(`[VSIX] list tasks failed (${response.status}): ${responseText}`);
                return;
            }
            let payload;
            try {
    ...
    const claimTask = vscode.commands.registerCommand("cocopilot.claimTask", async () => {
        const cocopilotConfig = vscode.workspace.getConfiguration("cocopilot");
        const apiBase = cocopilotConfig.get("apiBase") || "http://localhost:8080";
        const projectId = cocopilotConfig.get("projectId") || "";
        const agentId = cocopilotConfig.get("agentId") || "vsix-agent";
        if (!projectId.trim()) {
        ...
        try {
            ...
            const responseText = await response.text();
            if (!response.ok) {
                vscode.window.showErrorMessage(`Failed to claim task (${response.status}).`);
                outputChannel.appendLine(`[VSIX] claim task failed (${response.status}): ${responseText}`);
                return;
            }
            let envelope;
            try {
    ...
    const openSettings = vscode.commands.registerCommand("cocopilot.openSettings", async () => {
        await vscode.commands.executeCommand("workbench.action.openSettings", "cocopilot");
    });
    const showQuickStart = vscode.commands.registerCommand("cocopilot.showQuickStart", async () => {
        const selection = await vscode.window.showInformationMessage("Cocopilot quick start: configure MCP, start the MCP server, or open the dashboard.", "Configure MCP", "Start MCP", "Open Dashboard");
    ...
    const setProjectId = vscode.commands.registerCommand("cocopilot.setProjectId", async () => {
        const cocopilotConfig = vscode.workspace.getConfiguration("cocopilot");
        const current = cocopilotConfig.get("projectId") || "";
        ...
        updateStatusBar();
        if (trimmed) {
            vscode.window.showInformationMessage(`Cocopilot project ID set to ${trimmed}.`);
        }
        else {
            vscode.window.showInformationMessage("Cocopilot project ID cleared.");
        }
    });
    const setApiBase = vscode.commands.registerCommand("cocopilot.setApiBase", async () => {
        const cocopilotConfig = vscode.workspace.getConfiguration("cocopilot");
        const current = cocopilotConfig.get("apiBase") || "";
        const value = await vscode.window.showInputBox({
        ...
        if (trimmed) {
            vscode.window.showInformationMessage(`Cocopilot API base set to ${trimmed}.`);
        }
        else {
            vscode.window.showInformationMessage("Cocopilot API base cleared.");
        }
    });
    const openApiDocs = vscode.commands.registerCommand("cocopilot.openApiDocs", async () => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            vscode.window.showErrorMessage("Open a workspace folder first.");
            return;
        }
        const docsUri = vscode.Uri.joinPath(workspaceFolder.uri, "docs", "api", "README.md");
        const doc = await vscode.workspace.openTextDocument(docsUri);
        await vscode.window.showTextDocument(doc, { preview: false });
    });
    const openOpenApiSpec = vscode.commands.registerCommand("cocopilot.openOpenApiSpec", async () => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            vscode.window.showErrorMessage("Open a workspace folder first.");
            return;
        }
        const specUri = vscode.Uri.joinPath(workspaceFolder.uri, "docs", "api", "openapi-v2.yaml");
        const doc = await vscode.workspace.openTextDocument(specUri);
        await vscode.window.showTextDocument(doc, { preview: false });
    });
    const openApiSummary = vscode.commands.registerCommand("cocopilot.openApiSummary", async () => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            vscode.window.showErrorMessage("Open a workspace folder first.");
            return;
        }
        const summaryUri = vscode.Uri.joinPath(workspaceFolder.uri, "docs", "api", "v2-summary.md");
        const doc = await vscode.workspace.openTextDocument(summaryUri);
        await vscode.window.showTextDocument(doc, { preview: false });
    });
    const openApiDesign = vscode.commands.registerCommand("cocopilot.openApiDesign", async () => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            vscode.window.showErrorMessage("Open a workspace folder first.");
            return;
        }
        const designUri = vscode.Uri.joinPath(workspaceFolder.uri, "docs", "api", "v2-design.md");
        const doc = await vscode.workspace.openTextDocument(designUri);
        await vscode.window.showTextDocument(doc, { preview: false });
    });
    const openApiCompatibility = vscode.commands.registerCommand("cocopilot.openApiCompatibility", async () => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            vscode.window.showErrorMessage("Open a workspace folder first.");
            return;
        }
        const compatUri = vscode.Uri.joinPath(workspaceFolder.uri, "docs", "api", "v2-compatibility.md");
        const doc = await vscode.workspace.openTextDocument(compatUri);
        await vscode.window.showTextDocument(doc, { preview: false });
    });
    const openApiRoadmap = vscode.commands.registerCommand("cocopilot.openApiRoadmap", async () => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            vscode.window.showErrorMessage("Open a workspace folder first.");
            return;
        }
        const roadmapUri = vscode.Uri.joinPath(workspaceFolder.uri, "docs", "api", "v2-roadmap.md");
        const doc = await vscode.workspace.openTextDocument(roadmapUri);
        await vscode.window.showTextDocument(doc, { preview: false });
    });
    const openMigrationsDocs = vscode.commands.registerCommand("cocopilot.openMigrationsDocs", async () => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            vscode.window.showErrorMessage("Open a workspace folder first.");
            return;
        }
        const docsUri = vscode.Uri.joinPath(workspaceFolder.uri, "MIGRATIONS.md");
        const doc = await vscode.workspace.openTextDocument(docsUri);
        await vscode.window.showTextDocument(doc, { preview: false });
    });
    const openRoadmap = vscode.commands.registerCommand("cocopilot.openRoadmap", async () => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            vscode.window.showErrorMessage("Open a workspace folder first.");
            return;
        }
        const roadmapUri = vscode.Uri.joinPath(workspaceFolder.uri, "ROADMAP.md");
        const doc = await vscode.workspace.openTextDocument(roadmapUri);
        await vscode.window.showTextDocument(doc, { preview: false });
    });
    const openKbOverview = vscode.commands.registerCommand("cocopilot.openKbOverview", async () => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            vscode.window.showErrorMessage("Open a workspace folder first.");
            return;
        }
        const overviewUri = vscode.Uri.joinPath(workspaceFolder.uri, "docs", "ai", "kb", "00-overview.md");
        const doc = await vscode.workspace.openTextDocument(overviewUri);
        await vscode.window.showTextDocument(doc, { preview: false });
    });
    const openStateDocs = vscode.commands.registerCommand("cocopilot.openStateDocs", async () => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            vscode.window.showErrorMessage("Open a workspace folder first.");
            return;
        }
        const overviewUri = vscode.Uri.joinPath(workspaceFolder.uri, "docs", "state", "overview.md");
        const currentUri = vscode.Uri.joinPath(workspaceFolder.uri, "docs", "state", "current.md");
    ...
    const openStateArchitecture = vscode.commands.registerCommand("cocopilot.openStateArchitecture", async () => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            vscode.window.showErrorMessage("Open a workspace folder first.");
            return;
        }
        const architectureUri = vscode.Uri.joinPath(workspaceFolder.uri, "docs", "state", "architecture.md");
        const doc = await vscode.workspace.openTextDocument(architectureUri);
        await vscode.window.showTextDocument(doc, { preview: false });
    });
    const openStateDecisions = vscode.commands.registerCommand("cocopilot.openStateDecisions", async () => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            vscode.window.showErrorMessage("Open a workspace folder first.");
            return;
        }
        const decisionsUri = vscode.Uri.joinPath(workspaceFolder.uri, "docs", "state", "decisions.md");
        const doc = await vscode.workspace.openTextDocument(decisionsUri);
        await vscode.window.showTextDocument(doc, { preview: false });
    });
    const openStateCurrent = vscode.commands.registerCommand("cocopilot.openStateCurrent", async () => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            vscode.window.showErrorMessage("Open a workspace folder first.");
            return;
    ...
    const openStateNext = vscode.commands.registerCommand("cocopilot.openStateNext", async () => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            vscode.window.showErrorMessage("Open a workspace folder first.");
            return;
        }
        const nextUri = vscode.Uri.joinPath(workspaceFolder.uri, "docs", "state", "next.md");
        const doc = await vscode.workspace.openTextDocument(nextUri);
        await vscode.window.showTextDocument(doc, { preview: false });
    });
    const openStateRisks = vscode.commands.registerCommand("cocopilot.openStateRisks", async () => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            vscode.window.showErrorMessage("Open a workspace folder first.");
            return;
        }
        const risksUri = vscode.Uri.joinPath(workspaceFolder.uri, "docs", "state", "risks.md");
        const doc = await vscode.workspace.openTextDocument(risksUri);
        await vscode.window.showTextDocument(doc, { preview: false });
    });
    const openMcpReadme = vscode.commands.registerCommand("cocopilot.openMcpReadme", async () => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            vscode.window.showErrorMessage("Open a workspace folder first.");
            return;
        }
        const readmeUri = vscode.Uri.joinPath(workspaceFolder.uri, "tools", "cocopilot-mcp", "README.md");
        const doc = await vscode.workspace.openTextDocument(readmeUri);
        await vscode.window.showTextDocument(doc, { preview: false });
    });
    const openMcpTools = vscode.commands.registerCommand("cocopilot.openMcpTools", async () => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            vscode.window.showErrorMessage("Open a workspace folder first.");
            return;
        }
        const toolsUri = vscode.Uri.joinPath(workspaceFolder.uri, "tools", "cocopilot-mcp", "tools.json");
        const doc = await vscode.workspace.openTextDocument(toolsUri);
        await vscode.window.showTextDocument(doc, { preview: false });
    });
    const showLogs = vscode.commands.registerCommand("cocopilot.showLogs", () => {
    ...
    const startServer = vscode.commands.registerCommand("cocopilot.startServer", async () => {
        await vscode.commands.executeCommand("cocopilot.startMcpServer");
    });
    const stopServer = vscode.commands.registerCommand("cocopilot.stopServer", async () => {
        await vscode.commands.executeCommand("cocopilot.stopMcpServer");
    });
    const showTasks = vscode.commands.registerCommand("cocopilot.showTasks", async () => {
