...
func main() {
	...
	modelFiles := []string{
		filepath.Join(root, "internal", "models", "models.go"),
		filepath.Join(root, "server", "models_v2.go"),
		filepath.Join(root, "server", "assignment.go"),
	}
...
func generateTypes(path string, structs []goStruct, consts []goConst) error {
	...
	fmt.Fprintf(w, " * Source files: internal/models/models.go, server/models_v2.go, server/assignment.go\n")
	fmt.Fprintf(w, " */\n\n")
